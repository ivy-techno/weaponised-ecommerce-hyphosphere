...
function AgentBriefSurface({ onOpenExample, onOpenCollection, onOpenEvidence, onOpenOutputs, onSave }: { onOpenExample: (sourceId: string) => void; onOpenCollection: (sourceId: string) => void; onOpenEvidence: (nodeId: string) => void; onOpenOutputs: () => void; onSave: () => void }) {
  ...
  const runBrief = (requestedQuery: string, requestedScope = searchScope) => {
...
export default function Home() {
  ...
  const runAgentAction = useCallback((action: string, input: Record<string, unknown> = {}) => {
    ...
    if (action === 'search_research') {
      ...
      setSearch(query);
      return { ok: true, action, query, scope, scopeNote: scope === 'web' ? 'Live web import is not connected in this proof of concept; use the linked source index as a starting point.' : scope === 'curated' ? 'Results are from the reproducible local demonstration corpus.' : 'Results combine local demonstration records with the linked source index; no live web fetch is implied.', objects: scope === 'web' ? [] : matchingNodes.map((node) => ({ id: node.id, label: node.label, kind: node.kind, source: node.source })), externalSources: scope === 'curated' ? [] : matchingSources.map((source) => ({ id: source.id, title: source.title, provider: source.provider, url: source.url })) };
    }
  ...
  useEffect(() => {
    ...
    modelContext.registerTool({ name: 'search_research', description: 'Search the selected Hyphosphere research scope. Choose web for the link-ready web-source index, curated for local demonstration records, or both to cross-check them. Live web/database import is not connected in this proof of concept.', inputSchema: { type: 'object', properties: { query: { type: 'string', description: 'A research term, dataset name, provider, or source type.' }, scope: { type: 'string', enum: ['web', 'curated', 'both'], description: 'Search web, curated records, or both. Defaults to both.' } }, required: ['query'] } }, (input) => runAgentAction('search_research', input));
  }, [runAgentAction]);
