      ...
      <div className="workspace-grid">
        <aside className={`sidebar ${mobileNavOpen ? 'is-open' : ''}`}>
          <div className="sidebar-scroll">
            ...
            <button className="notebook-link" onClick={() => { changeView('thread'); announce(saved ? 'Your saved discovery is preserved in the notebook card below.' : 'Your notebook is empty. Start with Atlas Relay, then use Save discovery when you are ready.'); }}><Bookmark size={15} /> <span>Saved discoveries</span><small>{saved ? '01' : '00'}</small></button><button className="notebook-link" onClick={() => { changeView('terrain'); announce('Source ecology opened. Choose a layer to inspect its artifacts and external sources.'); }}><Archive size={15} /> <span>Source shelf</span><small>12</small></button>
          </div>
        ...
        <section className="main-stage">
          ...
          <div ref={stageContentRef} className="stage-content">
            {view === 'terrain' ? <TerrainView selectedLayer={selectedTerrainLayer} onFollow={() => followNode('service')} onOpenThread={() => changeView('thread')} onInspect={inspectTerrain} onOpenObject={openEvidence} onOpenExample={openExample} followed={followed} /> : view === 'evidence' ? <EvidenceView selected={selected} edges={edges} onOpen={openEvidence} onFollow={followNode} onOpenThread={() => changeView('thread')} followed={followed} evidenceOnly={evidenceOnly} /> : view === 'compare' ? <CompareView followed={followed} selectedId={selectedId} onFollow={() => { if (!followed) followNode('service'); changeView('thread'); }} onSelectCase={(id) => { setSelectedId(id); announce(`${nodeById(id).label} selected for comparison.`); }} /> : view === 'concepts' ? <CorpusView mode="concepts" selectedId={selectedId} onSelect={setSelectedId} onOpenEvidence={openEvidence} onOpenExample={openExample} onChangeMode={changeView} /> : view === 'artifacts' ? <CorpusView mode="artifacts" selectedId={selectedId} onSelect={setSelectedId} onOpenEvidence={openEvidence} onOpenExample={openExample} onChangeMode={changeView} /> : view === 'thread' ? <ThreadView selectedId={selectedId} followed={followed} evidenceOnly={evidenceOnly} onSelect={setSelectedId} onFollow={followNode} onOpenEvidence={openEvidence} onToggleVerified={toggleVerified} onChangeView={changeView} /> : <MapView nodes={filteredNodes} edges={filteredEdges} selectedId={selectedId} visibleNodeIds={visibleNodeIds} followed={followed} evidenceOnly={evidenceOnly} onSelect={setSelectedId} onFollow={followNode} onOpenThread={() => changeView('thread')} onOpenEvidence={openEvidence} onToggleVerified={toggleVerified} onOpenStack={() => setStackOpen(true)} />}
...
function MapView(props: { nodes: ResearchNode[]; edges: ResearchEdge[]; selectedId: string; visibleNodeIds: Set<string>; followed: boolean; evidenceOnly: boolean; onSelect: (id: string) => void; onFollow: (id: string) => void; onOpenThread: () => void; onOpenEvidence: (id: string) => void; onToggleVerified: () => void; onOpenStack: () => void }) {
  ...
  return <div ref={surfaceRef} className="map-surface-shell"><div className="map-explainer"><div className="map-explainer-icon"><Network size={19} /></div><div className="map-explainer-copy"><span className="eyebrow-label">WHAT THIS NETWORK IS SHOWING</span><strong>An information operation can look highly visible while occupying only a tiny layer of a giant ecommerce stack.</strong><p>Atlas Relay is the small connective service being tested here—not the whole operation. The lines show where that layer touches cases, datasets, platform artefacts, and infrastructure, with uncertainty carried by every relationship.</p></div><div className="map-stack-model" aria-label="From visible public story to buried commercial infrastructure"><span className="map-stack-label">VISIBLE → BURIED</span><div className="stack-step stack-visible"><b>01</b><span>Public story / IO</span></div><div className="stack-step stack-service"><b>02</b><span>Shared service / Atlas Relay</span></div><button type="button" className="stack-step stack-infrastructure stack-step-button" onClick={props.onOpenStack} aria-label="Open giant ecommerce stack explainer"><b>03</b><span>Giant ecommerce stack</span><ArrowUpRight size={13} /></button></div></div><MapCanvas {...props} /><aside ref={inspectorRef} className="map-inspector" aria-live="polite"><span className="eyebrow-label">SELECTED OBJECT</span><div className={`map-inspector-icon object-${selected.accent}`}><NodeIcon kind={selected.kind} /></div><strong>{selected.label}</strong><span>{selected.kind} · {selected.source}</span><div className="map-inspector-stack"><span>STACK POSITION</span><StackLayerPills layers={selected.stackLayers} /></div><EvidencePill state={selected.evidence} /><p>{selected.preview}</p><div className="map-inspector-actions"><button className="quiet-button" onClick={() => props.onOpenEvidence(selected.id)}>Inspect evidence</button><button className="follow-button" onClick={() => props.followed ? props.onOpenThread() : props.onFollow(selected.id)}><span className="follow-glyph"><ArrowUpRight size={14} /></span>{props.followed ? 'Open followed path' : 'Follow this'}</button></div></aside></div>;
}
...
function TerrainView({ selectedLayer, onFollow, onOpenThread, onInspect, onOpenObject, onOpenExample, followed }: { selectedLayer: TerrainLayer | null; onFollow: () => void; onOpenThread: () => void; onInspect: (label: string) => void; onOpenObject: (id: string) => void; onOpenExample: (id: string) => void; followed: boolean }) {
