...
const AGENT_RESEARCH_INSTRUCTION = 'Act as a careful research assistant. Search the linked data collections and current reporting for a focused question about information warfare and the ecommerce stack. Return a small candidate set and state the source, date or recurrence signal, geography, stack position, evidence status, and why each item is relevant. Keep direct observation, support, inference, and dispute separate. Ask the researcher which records to retain, then offer a mini report, timeline, threshold note, or network map. Do not turn shared infrastructure into attribution or proof. In this proof of concept, treat external links as starting points rather than imported or independently verified records.';
...
function ResearchPrimer() {
  ...
  return <section className="research-primer" aria-label="Orientation: information warfare and the ecommerce stack"><article className="primer-card primer-information"><span className="eyebrow-label">FIRST QUESTION</span><h2>What is information warfare?</h2><p className="primer-definition"><strong>Working definition:</strong> the deliberate use or manipulation of information, communication channels, and digital systems to influence what people believe, notice, or do in pursuit of political or strategic ends.</p><div className="primer-detail"><span>ONLINE FOCUS</span><strong>Posts, accounts, data, platforms, services, and infrastructure — not only one visible post.</strong></div></article><div className="primer-bridge" aria-label="The connection between information warfare and the ecommerce stack"><div className="primer-bridge-icon"><Link2 size={18} /></div><span>THE HIDDEN CONNECTION</span><p>Modern operations can borrow ordinary commercial rails. The infrastructure is part of daily life, but its role can be difficult to see and regulate.</p></div><article className="primer-card primer-stack"><span className="eyebrow-label">SECOND QUESTION</span><h2>What is the ecommerce stack?</h2><p>A layered commercial environment connecting visible activity to the less visible systems that make it possible.</p><div className="primer-stack-list" aria-label="Broad ecommerce stack categories">{primerLayers.map((layer) => <span key={layer}>{layer}</span>)}</div><div className="primer-detail"><span>WHY IT MATTERS</span><strong>Mostly ordinary, mostly invisible, and sometimes available for misuse across several thresholds.</strong></div></article></section>;
}

function AgentBriefSurface({ onOpenExample, onOpenCollection, onOpenOutputs, onSave }: { onOpenExample: (sourceId: string) => void; onOpenCollection: (sourceId: string) => void; onOpenOutputs: () => void; onSave: () => void }) {
  const [query, setQuery] = useState('');
  const [brief, setBrief] = useState<ResearchSource[] | null>(null);
  const [kept, setKept] = useState<string[]>([]);
  const [outputChoice, setOutputChoice] = useState('');

  const runBrief = (requestedQuery: string) => {
    ...
    const ranked = externalSources.map((source, index) => {
      const haystack = [source.title, source.provider, source.kind, source.description, source.whyIncluded, ...source.tags].join(' ').toLowerCase();
      const score = terms.reduce((total, term) => total + (haystack.includes(term) ? 1 : 0), 0);
      return { source, score, index };
    }).sort((left, right) => right.score - left.score || left.index - right.index).slice(0, 4).map(({ source }) => source);
    setQuery(nextQuery);
    setBrief(ranked);
    setKept(ranked.slice(0, Math.min(3, ranked.length)).map((source) => source.id));
    setOutputChoice('');
  };

  const toggleKept = (sourceId: string) => setKept((current) => current.includes(sourceId) ? current.filter((id) => id !== sourceId) : [...current, sourceId]);
  const getTimeSignal = (source: ResearchSource) => source.example.fields.find((field) => /date|time|period|range|year|observed|capture/i.test(field.label))?.value ?? 'recurrence to check in original source';
  ...
  return <section className="agent-brief-surface" aria-label="Agent-assisted research brief"><div className="agent-brief-head"><div><span className="eyebrow-label">WEBMCP RESEARCH BRIEF · CURATED DEMO</span><h2>Ask the wider field a question.</h2><p>Use a lucky prompt or write your own. The prototype searches its curated local records and linked collections, then turns a small candidate set into something you can inspect and retain.</p></div><div className="agent-brief-status"><Network size={18} /><span>LOCAL + LINKED SOURCES</span><small>Live imports are future observatory work.</small></div></div><details className="agent-instruction"><summary><span>AGENT INSTRUCTION</span><strong>How the research brief is composed</strong><ChevronRight size={14} /></summary><p>{AGENT_RESEARCH_INSTRUCTION}</p></details><form className="agent-brief-form" onSubmit={(event) => { event.preventDefault(); runBrief(query); }}><label className="agent-brief-field" htmlFor="terrain-search"><span>SEARCH THE TERRAIN</span><small>ask a specific question, or let the agent choose a starting point</small></label><div className="agent-brief-controls"><input id="terrain-search" value={query} onChange={(event) => setQuery(event.target.value)} placeholder="Try: what is the latest on info war + ecommerce?" /><button type="submit" className="primary-button"><Search size={14} /> Search</button><button type="button" className="quiet-button" onClick={() => runBrief('')}><Sparkles size={14} /> I’m feeling lucky</button></div><div className="agent-brief-suggestions" aria-label="Suggested research questions">{suggestions.map((suggestion) => <button type="button" className="agent-brief-suggestion" key={suggestion} onClick={() => runBrief(suggestion)}>{suggestion}<ChevronRight size={12} /></button>)}</div></form>{brief && <div className="agent-brief-results" aria-live="polite"><div className="agent-brief-results-head"><div><span className="eyebrow-label">RAPID FIELD SCAN</span><h3>{query}</h3></div><span>{brief.length} candidate records · {kept.length} retained</span></div><div className="agent-brief-result-layout"><div className="agent-brief-visual" aria-label={`Illustrative stack lens showing ${brief.length} candidate sources`}><div className="agent-brief-visual-core"><Network size={22} /><span>STACK LENS</span><strong>{kept.length} retained</strong><small>of {brief.length} candidates</small></div><div className="agent-brief-visual-links">{brief.map((source, index) => <div className="agent-brief-visual-link" key={source.id}><b>{(index + 1).toString().padStart(2, '0')}</b><span><strong>{source.example.label}</strong><small>{sourceStackLayers(source)[0]}</small></span></div>)}</div><p>Illustrative connections across source types — a lead to inspect, not an inferred causal chain.</p></div><div className="agent-brief-result-list">{brief.map((source, index) => { const isKept = kept.includes(source.id); return <div className={`agent-brief-result-row ${isKept ? 'is-kept' : ''}`} key={source.id}><button type="button" className="agent-brief-result-main" onClick={() => toggleKept(source.id)} aria-pressed={isKept}><span className="agent-brief-result-main-head"><b>{(index + 1).toString().padStart(2, '0')}</b><small>{source.kind} · {source.provider}</small><em>{isKept ? 'RETAINED' : 'SELECT TO RETAIN'}</em></span><strong>{source.title}</strong><StackLayerPills layers={sourceStackLayers(source)} /><span className="agent-brief-result-signal"><Clock3 size={12} /><span>signal: {getTimeSignal(source)}</span></span></button><div className="agent-brief-result-actions"><button type="button" onClick={() => onOpenExample(source.id)}>Example</button><button type="button" onClick={() => onOpenCollection(source.id)}>Profile</button><a href={source.url} target="_blank" rel="noreferrer" aria-label={`Open original source for ${source.title}`}><ExternalLink size={12} /></a></div></div>; })}</div></div><div className="agent-brief-followup"><div><span className="eyebrow-label">WHAT SHOULD THIS BECOME?</span><p>Select candidate records above, then choose a form for the next research note.</p></div><div className="agent-output-options">{outputOptions.map((option) => <button type="button" className={`agent-output-option ${outputChoice === option ? 'is-selected' : ''}`} key={option} onClick={() => { setOutputChoice(option); onOpenOutputs(); }}>{option}<ChevronRight size={12} /></button>)}<button type="button" className="agent-output-option" onClick={() => { setOutputChoice('Saved to notebook'); onSave(); }}>Save to notebook <Bookmark size={12} /></button>{outputChoice && <small className="agent-output-choice">{outputChoice} selected</small>}</div></div><p className="agent-brief-caveat"><CircleHelp size={14} /> The scan surfaces leads. It does not import every record or turn overlap into proof.</p></div>}</section>;
}
    ...
    <main className="app-shell" onClickCapture={handleSurfaceClick}>
      ...
      <div className="workspace-grid">
        ...
        <section className="main-stage">
          ...
          <ResearchPrimer />
          <AgentBriefSurface onOpenExample={openExample} onOpenCollection={openCollection} onOpenOutputs={() => changeView('outputs')} onSave={saveDiscovery} />
