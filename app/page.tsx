...
function ResearchPrimer() {
  ...
  return <section className="research-primer" aria-label="Orientation: information warfare and the ecommerce stack"><div className={`research-primer-junction research-primer-active-${activePrimer}`}><button type="button" className={`primer-card primer-information ${activePrimer === 'information' ? 'is-active' : ''}`} onClick={() => setActivePrimer('information')} aria-pressed={activePrimer === 'information'}><span className="primer-card-top"><span className="eyebrow-label">FIRST QUESTION</span><b>01</b></span><h2>What is information warfare?</h2><p className="primer-definition"><strong>Working definition:</strong> the deliberate use or manipulation of information, communication channels, and digital systems to influence belief, attention, or action for political or strategic ends.</p><div className="primer-detail"><span>ONLINE FOCUS</span><strong>Posts, accounts, adverts, takedowns, and the services or infrastructure behind them.</strong></div>{activePrimer === 'information' && <span className="primer-press-detail"><b>LOOK FOR</b> visible narratives, accounts, adverts, takedowns, and the traces they leave behind.</span>}<span className="primer-card-action">{activePrimer === 'information' ? 'Definition in focus' : 'Select to see the definition'} <ChevronRight size={13} /></span></button><button type="button" className={`primer-bridge ${activePrimer === 'connection' ? 'is-active' : ''}`} onClick={() => setActivePrimer('connection')} aria-pressed={activePrimer === 'connection'} aria-label="Focus the research junction between information warfare and the ecommerce stack"><div className="primer-bridge-icon"><Link2 size={18} /></div><span>THE RESEARCH JUNCTION</span><strong>Where an information operation meets the commercial systems that help it travel, scale, or persist</strong><p>The research follows links across cases, platforms, services, data, interfaces, and infrastructure.</p><span className="primer-bridge-action">{activePrimer === 'connection' ? 'Junction in focus' : 'Highlight the connection'} <ChevronRight size={13} /></span></button><button type="button" className={`primer-card primer-stack ${activePrimer === 'stack' ? 'is-active' : ''}`} onClick={() => setActivePrimer('stack')} aria-pressed={activePrimer === 'stack'}><span className="primer-card-top"><span className="eyebrow-label">SECOND QUESTION</span><b>02</b></span><h2>What is the ecommerce stack?</h2><p>The layered commercial systems that connect a visible service to the less visible tools, data, and infrastructure beneath it.</p><div className="primer-stack-list" aria-label="Broad ecommerce stack categories">{primerLayers.map((layer) => <span key={layer}>{layer}</span>)}</div><div className="primer-detail"><span>WHY IT MATTERS</span><strong>Mostly ordinary and often invisible; misuse can occur at several thresholds.</strong></div>{activePrimer === 'stack' && <span className="primer-press-detail"><b>LOOK FOR</b> the services, data, interfaces, and infrastructure that make activity persistent or scalable.</span>}<span className="primer-card-action">{activePrimer === 'stack' ? 'Definition in focus' : 'Select to see the definition'} <ChevronRight size={13} /></span></button></div></section>;
}
