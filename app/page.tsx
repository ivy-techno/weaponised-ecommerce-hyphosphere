...
const AGENT_RESEARCH_INSTRUCTION = 'Act as a careful research assistant. First use the source catalogue and stack filter when they help narrow the question; then search the linked data collections and current reporting for a focused question about information warfare and the ecommerce stack. Return a small candidate set and state the source URL, access or download note, date or recurrence signal, geography, stack position, evidence status, and why each item is relevant. Keep direct observation, support, inference, and dispute separate. Ask the researcher which records to retain, then offer a mini report, timeline, threshold note, or network map. Do not turn shared infrastructure into attribution or proof. In this proof of concept, treat external links as starting points rather than downloaded or independently verified records.';

function ResearchPrimer() {
  const primerLayers = ['platforms & distribution', 'commercial services', 'data & brokerage', 'interfaces & operations', 'domains, cloud & infrastructure'];
  const [activePrimer, setActivePrimer] = useState<'information' | 'connection' | 'stack'>('connection');
  return <section className="research-primer" aria-label="Orientation: information warfare and the ecommerce stack"><div className={`research-primer-junction research-primer-active-${activePrimer}`}><button type="button" className={`primer-card primer-information ${activePrimer === 'information' ? 'is-active' : ''}`} onClick={() => setActivePrimer('information')} aria-pressed={activePrimer === 'information'}><span className="primer-card-top"><span className="eyebrow-label">FIRST QUESTION</span><b>01</b></span><h2>What is information warfare?</h2><p className="primer-definition"><strong>Working definition:</strong> the deliberate use or manipulation of information, communication channels, and digital systems to influence what people believe, notice, or do in pursuit of political or strategic ends.</p><div className="primer-detail"><span>ONLINE FOCUS</span><strong>Posts, accounts, data, platforms, services, and infrastructure — not only one visible post.</strong></div>{activePrimer === 'information' && <span className="primer-press-detail"><b>LOOK FOR</b> visible narratives, accounts, adverts, takedowns, and the traces they leave behind.</span>}<span className="primer-card-action">{activePrimer === 'information' ? 'Definition selected' : 'Press to focus this question'} <ChevronRight size={13} /></span></button><button type="button" className={`primer-bridge ${activePrimer === 'connection' ? 'is-active' : ''}`} onClick={() => setActivePrimer('connection')} aria-pressed={activePrimer === 'connection'} aria-label="Focus the research junction between information warfare and the ecommerce stack"><div className="primer-bridge-icon"><Link2 size={18} /></div><span>THE RESEARCH JUNCTION</span><strong>Where the visible operation meets the enabling stack</strong><p>Modern operations can borrow ordinary commercial rails. The research follows this junction: how do cases, platforms, services, data, interfaces, and infrastructure connect?</p><span className="primer-bridge-action">{activePrimer === 'connection' ? 'Junction in focus' : 'Press to highlight the connection'} <ChevronRight size={13} /></span></button><button type="button" className={`primer-card primer-stack ${activePrimer === 'stack' ? 'is-active' : ''}`} onClick={() => setActivePrimer('stack')} aria-pressed={activePrimer === 'stack'}><span className="primer-card-top"><span className="eyebrow-label">SECOND QUESTION</span><b>02</b></span><h2>What is the ecommerce stack?</h2><p>A layered commercial environment connecting visible activity to the less visible systems that make it possible.</p><div className="primer-stack-list" aria-label="Broad ecommerce stack categories">{primerLayers.map((layer) => <span key={layer}>{layer}</span>)}</div><div className="primer-detail"><span>WHY IT MATTERS</span><strong>Mostly ordinary, mostly invisible, and sometimes available for misuse across several thresholds.</strong></div>{activePrimer === 'stack' && <span className="primer-press-detail"><b>LOOK FOR</b> the services, data, interfaces, and infrastructure that make activity persistent or scalable.</span>}<span className="primer-card-action">{activePrimer === 'stack' ? 'Definition selected' : 'Press to focus this question'} <ChevronRight size={13} /></span></button></div></section>;
}
...
export default function Home() {
  ...
  const [savedAgentBriefs, setSavedAgentBriefs] = useState<SavedAgentBrief[]>(() => {
    ...
    try {
      const stored = window.localStorage.getItem('hyphosphere-agent-briefs');
  ...
  const openTerrainAgentSearch = useCallback((query: string) => {
    ...
    setAgentMessage('Broader agent search opened from the Source Terrain field.');
  ...
  const runAgentAction = useCallback((action: string, input: Record<string, unknown> = {}) => {
    ...
    if (action === 'search_research') {
      ...
      const matchingNodes = nodes.filter((node) => (stackLayer === 'all' || node.stackLayers.includes(stackLayer)) && `${node.label} ${node.kind} ${node.source} ${node.preview} ${node.inclusionReason} ${node.stackLayers.join(' ')}`.toLowerCase().includes(normalizedQuery));
      const matchingSources = externalSources.filter((source) => (stackLayer === 'all' || sourceStackLayers(source).includes(stackLayer)) && `${source.title} ${source.provider} ${source.kind} ${source.description} ${source.whyIncluded} ${source.tags.join(' ')} ${sourceStackLayers(source).join(' ')}`.toLowerCase().includes(normalizedQuery));
      ...
      setView('thread');
      setSearch('');
      setAgentMessage(`Agent research brief opened for ${scope === 'both' ? 'local and linked sources' : scope === 'web' ? 'linked web sources' : 'curated records'}.`);
...
function TerrainView({ selectedLayer, onFollow, onOpenThread, onInspect, onOpenObject, onOpenExample, onOpenCollection, onOpenAgentSearch, followed }: { selectedLayer: TerrainLayer | null; onFollow: () => void; onOpenThread: () => void; onInspect: (label: string) => void; onOpenObject: (id: string) => void; onOpenExample: (id: string) => void; onOpenCollection: (id: string) => void; onOpenAgentSearch: (query: string) => void; followed: boolean }) {
  ...
  return (
    <div className="terrain-view">
      ...
      <section className="terrain-search-panel" aria-label="Live source terrain category search"><div className="terrain-search-head"><div><span className="eyebrow-label">LIVE CATEGORY SEARCH</span><strong>Find a related source category.</strong><p>Search updates as you type across the six layers, their demo objects, and linked collections.</p></div><span className="terrain-search-count" aria-live="polite">{matchingTerrainItems.length.toString().padStart(2, '0')} / 06 layers</span></div><div className="terrain-search-controls"><div className="terrain-search-field"><Search size={15} /><input id="source-terrain-search" value={terrainSearch} onChange={(event) => setTerrainSearch(event.target.value)} placeholder="Try: datasets, reporting, infrastructure…" aria-label="Search source terrain categories, objects, and collections" />{terrainSearch && <button type="button" className="terrain-search-clear" onClick={() => setTerrainSearch('')} aria-label="Clear source terrain search"><X size={13} /></button>}</div><span className="terrain-search-status">{terrainSearch.trim() ? `${matchingTerrainItems.length} matching categor${matchingTerrainItems.length === 1 ? 'y' : 'ies'} · local index` : 'Type to filter categories · local index, not a live web fetch'}</span></div><div className="terrain-search-quick" role="group" aria-label="Quick source terrain category searches"><span>TRY A CATEGORY</span>{quickSearches.map((term) => <button type="button" key={term} className={terrainSearch.toLowerCase() === term ? 'is-active' : ''} onClick={() => setTerrainSearch(term)} aria-pressed={terrainSearch.toLowerCase() === term}>{term}</button>)}<button type="button" className="terrain-agent-search-button" onClick={() => onOpenAgentSearch(terrainSearch)}><Sparkles size={12} /> Broader agent search <ChevronRight size={12} /></button></div><p className="terrain-search-note">Category search is local and updates instantly. Use the broader agent search to cross-check this term against the curated and link-ready source index.</p></section>
