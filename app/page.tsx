...
function ResearchPrimer() {
  ...
  return <section className="research-primer" aria-label="Orientation: information warfare and the ecommerce stack"><div className={`research-primer-junction research-primer-active-${activePrimer}`}><button type="button" className={`primer-card primer-information ${activePrimer === 'information' ? 'is-active' : ''}`} onClick={() => setActivePrimer('information')} aria-pressed={activePrimer === 'information'}><span className="primer-card-top"><b>01</b><span><span className="eyebrow-label">QUESTION ONE</span><small>VISIBLE INFORMATION OPERATION</small></span></span><h2>What is information warfare?</h2><p className="primer-definition"><strong>Working definition:</strong> the deliberate use or manipulation of information, communication channels, and digital systems to influence belief, attention, or action for political or strategic ends.</p><div className="primer-detail"><span>START WITH WHAT IS VISIBLE</span><strong>Posts, adverts, accounts, headlines, and takedowns — the public-facing traces of an operation.</strong></div><span className={`primer-press-detail ${activePrimer === 'information' ? 'is-visible' : ''}`} aria-hidden={activePrimer !== 'information'}><b>LOOK FOR</b> what the message is trying to change, and what traces it leaves behind.</span><span className="primer-card-action">Focus this definition <ChevronRight size={13} /></span></button><button type="button" className={`primer-bridge ${activePrimer === 'connection' ? 'is-active' : ''}`} onClick={() => setActivePrimer('connection')} aria-pressed={activePrimer === 'connection'} aria-label="Focus the research junction between information warfare and the ecommerce stack"><div className="primer-bridge-icon"><Link2 size={18} /></div><span>THE RESEARCH JUNCTION</span><strong>Where an information operation meets the commercial systems that help it travel, scale, or persist</strong><p>The research follows links across cases, platforms, services, data, interfaces, and infrastructure.</p><span className="primer-bridge-action">Focus this junction <ChevronRight size={13} /></span></button><button type="button" className={`primer-card primer-stack ${activePrimer === 'stack' ? 'is-active' : ''}`} onClick={() => setActivePrimer('stack')} aria-pressed={activePrimer === 'stack'}><span className="primer-card-top"><b>02</b><span><span className="eyebrow-label">QUESTION TWO</span><small>ENABLING COMMERCIAL INFRASTRUCTURE</small></span></span><h2>What is the ecommerce stack?</h2><p>The layered commercial infrastructure through which a digital operation can be hosted, distributed, targeted, automated, and measured.</p><div className="primer-stack-list" aria-label="Broad ecommerce stack categories">{primerLayers.map((layer) => <span key={layer}>{layer}</span>)}</div><div className="primer-detail"><span>START WITH THE LESS VISIBLE</span><strong>Mostly ordinary systems, often hidden from view; the research asks where opacity or misuse may occur.</strong></div><span className={`primer-press-detail ${activePrimer === 'stack' ? 'is-visible' : ''}`} aria-hidden={activePrimer !== 'stack'}><b>LOOK FOR</b> the services, data, interfaces, and infrastructure that make activity persistent or scalable.</span><span className="primer-card-action">Focus this definition <ChevronRight size={13} /></span></button></div></section>;
}
      ...
      <div className="workspace-grid">
        <aside className={`sidebar ${mobileNavOpen ? 'is-open' : ''}`}>
          <div className="sidebar-scroll">
            ...
            <nav className="view-nav" aria-label="Investigations"><button className={view === 'investigations' ? 'is-active' : ''} onClick={() => changeView('investigations')}><Waypoints size={16} /> <span>Investigation paths</span><small>02</small></button></nav>
