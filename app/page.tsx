    ...
    <main className="app-shell">
      ...
      <div className="workspace-grid">
        ...
        <section className="main-stage">
          <div className="stage-heading"><div><div className="eyebrow"><span>INVESTIGATION 01</span><span className="eyebrow-line" /><span>GUIDED DEMO</span></div><h1>Follow one clue across many sources.</h1><p>Hyphosphere helps you test whether an ordinary service connects otherwise different cases. Atlas Relay is selected—follow it, inspect the evidence, and save what you can defend.</p></div><div className="stage-heading-actions"><button className={`quiet-button ${evidenceOnly ? 'is-selected' : ''}`} onClick={toggleVerified}><Filter size={15} /> {evidenceOnly ? 'Verified only' : 'Full terrain'}</button><button className="primary-button" onClick={saveDiscovery}><Bookmark size={15} /> {saved ? 'Saved' : 'Save discovery'}</button></div></div>
          <div className="orientation-panel"><div className="orientation-copy"><span className="eyebrow-label">START HERE</span><strong>{followed ? 'The path is open.' : 'Your task is simple: follow the highlighted relationship.'}</strong><p>{followed ? 'Atlas Relay has revealed two more objects. Check what supports each connection before you save the finding.' : 'The highlighted Atlas Relay node is the clue. Click Follow this to reveal where it appears next.'}</p><span className="orientation-agent-note">WEBMCP IDEA · one shared state for human and agent actions</span></div><div className="orientation-steps"><div><b>01</b><span>Choose a clue<small>Atlas Relay is selected</small></span></div><div><b>02</b><span>Follow it<small>Reveal related objects</small></span></div><div><b>03</b><span>Check evidence<small>Keep uncertainty visible</small></span></div><div><b>04</b><span>Save a finding<small>Export the trail</small></span></div></div><button className="orientation-cta" onClick={() => { setView('thread'); followNode('service'); }}>{followed ? 'Continue the path' : 'Start guided investigation'} <ChevronRight size={15} /></button></div>
