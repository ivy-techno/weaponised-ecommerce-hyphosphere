...
function AgentBriefSurface({ onOpenExample, onOpenCollection, onOpenEvidence, onOpenOutputs, onSave }: { onOpenExample: (sourceId: string) => void; onOpenCollection: (sourceId: string) => void; onOpenEvidence: (nodeId: string) => void; onOpenOutputs: () => void; onSave: () => void }) {
  ...
  const localRecords: AgentBriefRecord[] = nodes.map((node) => ({ id: `node:${node.id}`, title: node.label, kind: node.kind, provider: node.source, description: node.preview, stackLayers: node.stackLayers, signal: node.meta, node }));
  const linkedRecords: AgentBriefRecord[] = externalSources.map((source) => ({ id: `source:${source.id}`, title: source.title, kind: source.kind, provider: source.provider, description: source.description, stackLayers: sourceStackLayers(source), signal: source.example.fields.find((field) => /date|time|period|range|year|observed|capture/i.test(field.label))?.value ?? 'recurrence to check in original source', source }));
  const runBrief = (requestedQuery: string, requestedScope = searchScope, requestedStackFilter: StackLayer | 'all' = stackFilter) => {
    ...
    const ranked = Array.from(new Map(mixed.map((record) => [record.id, record])).values()).slice(0, 4);
    ...
    <main className="app-shell" onClickCapture={handleSurfaceClick}>
      ...
      <div className="workspace-grid">
        ...
        <section className="main-stage">
          ...
          <ResearchPrimer />
          <AgentBriefSurface onOpenExample={openExample} onOpenCollection={openCollection} onOpenEvidence={openEvidence} onOpenOutputs={() => changeView('outputs')} onSave={saveDiscovery} />
